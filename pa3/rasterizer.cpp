...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    const auto& vertice = t.toVector4();
    for (const auto& v: vertice) 
    {
        minx = std::min<int>(v.x(), minx);
        miny = std::min<int>(v.y(), miny);
        maxx = std::max<int>(v.x()+1, maxx);
        maxy = std::max<int>(v.y()+1, maxy);
    }
    const auto v = view_pos.data();
    const auto v_vp = vertice.data();
    for (int x=minx;x<maxx;++x)
    {
        for (int y=miny;y<maxy;++y)
        {
            if (insideTriangle(x, y, v_vp))
            {
                auto ind = get_index(x, y);
                auto [alpha, beta, gamma] = computeBarycentric2D(x, y, v_vp);
                float Z = -1.0 / (alpha / v[0].z() + beta / v[1].z() + gamma / v[2].z());
                float zp = alpha * v[0].z() / v[0].z() + beta * v[1].z() / v[1].z() + gamma * v[2].z() / v[2].z();
                zp *= Z;
                if (depth_buf[ind] > zp)
                {
                    depth_buf[ind] = zp;
                    ...
                    interpolated_texcoords.x() = std::clamp<float>(interpolated_texcoords.x(), 0, 1);
                    interpolated_texcoords.y() = std::clamp<float>(interpolated_texcoords.y(), 0, 1);
                    auto interpolated_shadingcoords = interpolate(alpha, beta, gamma, v[0], v[1], v[2], 1.0f);
                    fragment_shader_payload payload(interpolated_shadingcoords, 
                        interpolated_color, 
                        interpolated_normal, 
